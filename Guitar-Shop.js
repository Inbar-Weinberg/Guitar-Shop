...
function ClassicGuitar(
  manufactureYear = DEF_MAN_YEAR,
  brand = DEF_BRAND,
  price = DEF_PRICE,
  numOfString = DEF_STRINGS,
  used = DEF_USED
) {
  this.manufactureYear = isInRange(manufactureYear, 1900, DEF_MAN_YEAR)
    ? manufactureYear
    : DEF_MAN_YEAR;
...
function isInRange(num, min = 0, max = Infinity) {
  num = parseFloat(num);
  if (Number.isFinite && num > min && num < max) return num;
  return false;
}
